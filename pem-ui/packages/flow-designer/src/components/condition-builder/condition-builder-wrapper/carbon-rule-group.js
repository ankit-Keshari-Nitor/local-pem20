...
import { isRuleGroup, isRuleGroupType, pathsAreEqual, TestID, useRuleGroup, useStopEventPropagation } from 'react-querybuilder';

export const CarbonRuleGroup = (props) => {
  const rg = useRuleGroup(props);

  const {
    schema: {
      controls: { ruleGroupBodyElements: RuleGroupBodyElements, ruleGroupHeaderElements: RuleGroupHeaderElements }
    }
  } = rg;
...
export const RuleGroupHeaderComponents = (rg) => {
  const {
    schema: {
      controls: {
...
export const RuleGroupBodyComponents = (rg) => {
  const {
    schema: {
      controls: {
        combinatorSelector: CombinatorSelectorControlElement,
  ...
  return rg.ruleGroup.rules.map((r, idx, { length: ruleArrayLength }) => {
    ...
    return (
      <React.Fragment key={key}>
        {idx > 0 && !rg.schema.independentCombinators && rg.schema.showCombinatorsBetweenRules && (
